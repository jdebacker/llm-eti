...
#   5. Expand rows by round(tax_unit_weight / 10), minimum 1
...
for year, filename in YEARS.items():
    ...
    with h5py.File(path, "r") as f:
        tax_unit_weight  = f["tax_unit_weight"][:]
    ...
    df = pd.DataFrame({"tax_unit_id": tax_unit_id, "tax_unit_weight": tax_unit_weight})
    df["broad_income"]   = df["tax_unit_id"].map(broad_tu)
    df["taxable_income"] = taxable_tu
    df["mtr"]            = df["tax_unit_id"].map(mtr_tu)
    df["year"]           = year
    df = df[df["tax_unit_weight"] > 0].dropna(subset=["tax_unit_weight"])
    ...
    df["repeat_count"] = df["tax_unit_weight"].apply(lambda w: max(round(w / 10), 1))
...
# ── Step 6: Export ─────────────────────────────────────────────────────────
export_cols = ["year", "tax_unit_id", "broad_income", "taxable_income", "mtr", "mtr_prime"]
