...
class TestModelResponses:
    ...
    def test_gpt_provides_numeric_response(self):
        ...
        assert (
            0 < result["taxable_income_this"] <= 50000
        )  # Should be positive and <= broad income
    ...
    def test_model_responds_to_tax_incentive(self):
        ...
        avg_income_increase = sum(
            r["taxable_income_this"] for r in increase_results
        ) / len(increase_results)
        avg_income_decrease = sum(
            r["taxable_income_this"] for r in decrease_results
        ) / len(decrease_results)
        ...
        assert (
            avg_income_increase < 37500
        ), "Expected lower income when tax rate increases"
        assert (
            avg_income_decrease > 37500
        ), "Expected higher income when tax rate decreases"
